...
Bool ActionManager::canGetRepairedAt( const Object *obj, const Object *repairDest, CommandSourceType commandSource ) 
{
	...
	if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			OH_BitTest( repairDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canTransferSuppliesAt( const Object *obj, const Object *transferDest ) 
{
	...
	// nothing can be done with things that are under construction
	if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			OH_BitTest( transferDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canGetHealedAt( const Object *obj, const Object *healDest, CommandSourceType commandSource ) 
{
	...
	// nothing can be done with things that are under construction
	if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			OH_BitTest( healDest->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
...
Bool ActionManager::canRepairObject( const Object *obj, const Object *objectToRepair, CommandSourceType commandSource ) 
{
	...
	// nothing can be done with things that are under construction
	if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			OH_BitTest( objectToRepair->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
		return FALSE;
																						 ...
																						 CommandSourceType commandSource )
{
	...
	if( OH_BitTest( objectBeingConstructed->getStatusBits(), 
...
Bool ActionManager::canEnterObject( const Object *obj, const Object *objectToEnter, CommandSourceType commandSource, CanEnterType mode )
{
	...
	// nothing can be done with things that are under construction
	if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE ||
			OH_BitTest( objectToEnter->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
	{
		return FALSE;
	}
...
//		return FALSE;
	...
	if( OH_BitTest( objectToHack->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == TRUE )
	{
		return FALSE;
	}
		...
		switch( spTemplate->getSpecialPowerType() )
		{
			...
			case SPECIAL_CASH_HACK:
				...
				if( target->isKindOf( KINDOF_STRUCTURE ) && r == ENEMIES )
				{
					...
					if( OH_BitTest( target->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) )
					{
						return FALSE;
					}
