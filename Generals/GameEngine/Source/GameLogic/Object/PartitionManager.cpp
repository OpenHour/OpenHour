																		...
																		Coord3D *result )
{
	...
	//
	if( OH_BitTest( options->flags, FPF_IGNORE_WATER ) == FALSE )
	{
		...
		//
		if( OH_BitTest( options->flags, FPF_WATER_ONLY ) && (isUnderwater == FALSE || layer != LAYER_GROUND) )
			return FALSE;
	...
	// object checks
	if( OH_BitTest( options->flags, FPF_IGNORE_ALL_OBJECTS ) == FALSE )
	{
...
//	Bool overlap = FALSE;
		...
		for( Object *them = iter->first(); them; them = iter->next() )
		{
			...
			if( options->relationshipObject )
			{
				...
				if( OH_BitTest( options->flags, FPF_IGNORE_ALLY_OR_NEUTRAL_UNITS ) == TRUE &&
				...
				if( OH_BitTest( options->flags, FPF_IGNORE_ALLY_OR_NEUTRAL_STRUCTURES ) == TRUE &&
						options->relationshipObject->getRelationship( them ) != ENEMIES &&
						them->isKindOf( KINDOF_STRUCTURE ) )
					continue;

				// if this is an enemy unit and we ignore those, do so
				if( OH_BitTest( options->flags, FPF_IGNORE_ENEMY_UNITS ) == TRUE &&
						options->relationshipObject->getRelationship( them ) == ENEMIES &&
						(them->isKindOf( KINDOF_INFANTRY ) || them->isKindOf( KINDOF_VEHICLE )) )
					continue;

				// if this is an enemy structure and we ignore those, do so
				if( OH_BitTest( options->flags, FPF_IGNORE_ENEMY_STRUCTURES ) == TRUE &&
						options->relationshipObject->getRelationship( them ) == ENEMIES &&
						them->isKindOf( KINDOF_STRUCTURE ) )
					continue;
																					 ...
																					 Coord3D *result )
{
	...
	DEBUG_ASSERTCRASH( !(OH_BitTest( options->flags, FPF_IGNORE_WATER ) == TRUE &&
										   OH_BitTest( options->flags, FPF_WATER_ONLY ) == TRUE),
...
void PartitionManager::doShroudReveal(Real centerX, Real centerY, Real radius, PlayerMaskType playerMask) 
{
	...
	for( Int currentIndex = ThePlayerList->getPlayerCount() - 1; currentIndex >=0; currentIndex-- )
	{
		...
		if( OH_BitTest( playerMask, currentPlayer->getPlayerMask() ) )
		{
...
void PartitionManager::getMostValuableLocation( Int playerIndex, UnsignedInt whichPlayerTypes, ValueOrThreat valType, Coord3D *outLocation )
{
	...
	for (i = 0; i < cellCount; ++i) {
		...
		for (Int player = 0; player < MAX_PLAYER_COUNT; ++player) {
			if (OH_BitTest(allPlayerMasks[player], playerMask)) {
...
Bool PartitionFilterStealthedAndUndetected::allow( Object *objOther )
{
	...
	if( OH_BitTest( objOther->getStatusBits(), OBJECT_STATUS_STEALTHED ) && !OH_BitTest( objOther->getStatusBits(), OBJECT_STATUS_DETECTED ) )
	{
	...
	else
	{
		...
		if( contain )
		{
			...
			if( contain->getStealthUnitsContained() == contain->getContainCount() )
			{
				...
				Object *member = it != contain->getContainedItemsList()->end() ? (*it) : nullptr;
				if( member && !OH_BitTest( (*it)->getStatusBits(), OBJECT_STATUS_DETECTED ) )
				{
...
static int cellValueProc(PartitionCell* cell, void* userData)
{
	...
	UnsignedInt val = 0;
	for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
		if (OH_BitTest(parms->allowedPlayersMasks, parms->allPlayersMask[i])) {
