																						...
																						WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(window->winGetParent() && OH_BitTest(window->winGetParent()->winGetStyle(),GWS_HORZ_SLIDER) )
			{
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			//
			if( OH_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
		...
		case GWM_LEFT_DOWN:
		{
			...
			if( OH_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
			{
				...
				if( OH_BitTest( instData->m_state, WIN_STATE_SELECTED ) )
					BitClear( instData->m_state, WIN_STATE_SELECTED );
				else
		...
		case GWM_LEFT_UP:
		{
			...
			//
			if( OH_BitTest( instData->getState(), WIN_STATE_SELECTED ) &&
					OH_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
			{
		...
		case GWM_RIGHT_DOWN:
		{
			...
			if( OH_BitTest( instData->getStatus(), WIN_STATUS_RIGHT_CLICK ) )
			{
...
void GadgetCheckLikeButtonSetVisualCheck( GameWindow *g, Bool checked )
{
	...
	if( OH_BitTest( g->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
	{
