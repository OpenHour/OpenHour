...
GameWindow *GameWindow::findPrevLeaf( void )
{
	...
	if( leaf->m_prev ) 
	{
		...
		while( leaf->m_child && 
					 OH_BitTest( leaf->m_status, WIN_STATUS_TAB_STOP ) == FALSE ) 
		{
...
GameWindow *GameWindow::findNextLeaf( void )
{
	...
	if( leaf->m_next ) 
	{
		...
		for( leaf = leaf->m_next; leaf; leaf = leaf->m_child )
			if( leaf->m_child == NULL || OH_BitTest( leaf->m_status, 
																						WIN_STATUS_TAB_STOP ) )
				return leaf;
			...
			if( leaf->m_parent && leaf->m_next ) 
			{
				...
				for( leaf = leaf->m_next; leaf; leaf = leaf->m_child )
					if( leaf->m_child == NULL || 
							OH_BitTest( leaf->m_status, WIN_STATUS_TAB_STOP ) )
						return leaf;
...
Int GameWindow::winHide( Bool hide )
{
	...
	if( hide )
	{
		...
		//
		if( !OH_BitTest( m_status, WIN_STATUS_NO_FLUSH ) )
			freeImages();
...
Bool GameWindow::winIsHidden( void )
{
	...
	return OH_BitTest( m_status, WIN_STATUS_HIDDEN );
...
void GameWindow::winSetFont( GameFont *font )
{
	...
	if( OH_BitTest( m_instData.getStyle(), GWS_SCROLL_LISTBOX ) )
		GadgetListBoxSetFont( this, font );
	else if( OH_BitTest( m_instData.getStyle(), GWS_COMBO_BOX ) )
		GadgetComboBoxSetFont( this, font );
	else if( OH_BitTest( m_instData.getStyle(), GWS_ENTRY_FIELD ) )
		GadgetTextEntrySetFont( this, font );
	else if( OH_BitTest( m_instData.getStyle(), GWS_STATIC_TEXT ) )
...
void GameWindow::winSetEnabledTextColors( Color color, Color borderColor )
{
	...
	if( OH_BitTest( m_instData.getStyle(), GWS_COMBO_BOX ) )
...
Int GameWindow::winDrawWindow( void )
{
	...
	if( OH_BitTest( m_status, WIN_STATUS_HIDDEN ) == FALSE && m_draw )
...
GameWindow *GameWindow::winPointInChild( Int x, Int y, Bool ignoreEnableCheck, Bool playDisabledSound )
{
	...
	for( child = m_child; child; child = child->m_next ) 
	{
		...
		{
			Bool enabled = ignoreEnableCheck || OH_BitTest( child->m_status, WIN_STATUS_ENABLED );
			Bool hidden = OH_BitTest( child->m_status, WIN_STATUS_HIDDEN );
			if( !hidden )
			{
				if( enabled )
				{
...
GameWindow *GameWindow::winPointInAnyChild( Int x, Int y, Bool ignoreHidden, Bool ignoreEnableCheck )
{
	...
	for( child = m_child; child; child = child->m_next ) 
	{
				...
				y >= origin.y && y <= origin.y + child->m_size.y )
		{
			...
			if( !(ignoreHidden == TRUE &&	OH_BitTest( child->m_status, WIN_STATUS_HIDDEN )) )
