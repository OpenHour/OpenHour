...
Bool CommandButton::isValidToUseOn(const Object *sourceObj, const Object *targetObj, const Coord3D *targetLocation, CommandSourceType commandSource) const
{
	...
	if( OH_BitTest( m_options, COMMAND_OPTION_NEED_OBJECT_TARGET ) && !targetObj ) 
	{
		return false;
	}

	if( OH_BitTest( m_options, NEED_TARGET_POS ) && !targetLocation ) 
	{
		return false;
	}
	
	if( OH_BitTest( m_options, COMMAND_OPTION_NEED_OBJECT_TARGET ) ) 
	{
		return TheActionManager->canDoSpecialPowerAtObject( sourceObj, targetObj, commandSource, m_specialPower, m_options, false );
	}

	if( OH_BitTest( m_options, NEED_TARGET_POS ) ) 
	{
...
{
	return OH_BitTest( m_options, CONTEXTMODE_COMMAND );
}
...
void ControlBar::evaluateContextUI( void )
{
	...
	else if ( drawToEvaluateFor )// either we have exactly one drawable, or we have isolated one to evaluate for...
	{
		...
		if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_SOLD ) )
			return;
		...
		if( OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) )
		{
...
void ControlBar::setControlCommand( GameWindow *button, const CommandButton *commandButton )
{
	...
	//
	if( OH_BitTest( commandButton->getOptions(), CHECK_LIKE ))
...
void ControlBar::triggerRadarAttackGlow( void )
{
	...
	if(OH_BitTest(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED) == TRUE)
...
void ControlBar::updateRadarAttackGlow ( void )
{
	...
	{
		m_radarAttackGlowWindow->winEnable(!OH_BitTest(m_radarAttackGlowWindow->winGetStatus(),WIN_STATUS_ENABLED));
	}
...
void ControlBar::populateSpecialPowerShortcut( Player *player)
{
	...
	for( i = 0; i < m_currentlyUsedSpecialPowersButtons; i++ )
	{
		...
		else
		{
				...
				//
				if( OH_BitTest( commandButton->getOptions(), NEED_SPECIAL_POWER_SCIENCE ) )
				{
