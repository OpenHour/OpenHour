...
Bool GameWindowManager::isEnabled( GameWindow *win )
{
	...
	if( OH_BitTest( win->m_status, WIN_STATUS_ENABLED ) == FALSE )
	{
		return FALSE;
	}
...
Bool GameWindowManager::isHidden( GameWindow *win )
{
	...
	if( OH_BitTest( win->m_status, WIN_STATUS_HIDDEN ))
	{
		return TRUE;
	}
																					...
																					WindowMsgData mData2 )
{
	...
	if( msg != GWM_DESTROY && OH_BitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return MSG_IGNORED;
...
Int GameWindowManager::winSetFocus( GameWindow *window )
{
	...
	if( window )
		if( OH_BitTest( window->winGetStatus(), WIN_STATUS_NO_FOCUS) )
			return 0;
		...
		if( m_grabWindow )
		{
			...
			switch( msg )
			{
				...
				case GWM_LEFT_UP:
				{
					...
					else if( OH_BitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ))
					{
						winSendInputMsg( m_grabWindow, GWM_LEFT_UP, packedMouseCoords, 0 );
					}
				...
				case GWM_LEFT_DRAG:
				{
					...
					if( OH_BitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ) )
					{
		...
		else
		{
			...
			else
			{
				...
				for( window = m_windowList; window; window = window->m_next )
				{
					...
					if( OH_BitTest( window->m_status, WIN_STATUS_ABOVE ) &&
							!OH_BitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
							...
							mousePos->y <= window->m_region.hi.y)
					{
						...
						if( OH_BitTest( window->m_status, WIN_STATUS_ENABLED ) )
						{
				...
				if( window == NULL )
				{
					...
					for( window = m_windowList; window; window = window->m_next )
					{
						...
						if( OH_BitTest( window->m_status, WIN_STATUS_BELOW ) &&
								!OH_BitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
								mousePos->x >= window->m_region.lo.x &&
								mousePos->x <= window->m_region.hi.x &&
								mousePos->y >= window->m_region.lo.y &&
			...
			if( window )
				if( OH_BitTest( window->m_status, WIN_STATUS_NO_INPUT ) )
				{
					if(window->winGetParent() && OH_BitTest( window->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
						window = window->winGetParent();
					else
						window = NULL;
				}
...
//							WinActivate( tempWin );
			...
			if( tooltipsOn )
			{
//				if( toolTipWindow && toolTipWindow->winGetParent() && OH_BitTest( toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
...
Int GameWindowManager::drawWindow( GameWindow *window )
{
	...
	if( OH_BitTest( window->m_status, WIN_STATUS_HIDDEN ) == FALSE )
	{

		if( !OH_BitTest( window->m_status, WIN_STATUS_SEE_THRU ) && window->m_draw )
		...
		if( OH_BitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
			if( OH_BitTest( window->m_status, WIN_STATUS_BORDER ) == TRUE &&
					!OH_BitTest( window->m_status, WIN_STATUS_SEE_THRU ) )
...
Int GameWindowManager::winDestroy( GameWindow *window )
{
	...
	if( OH_BitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return WIN_ERR_OK;
																										 ...
																										 Bool defaultVisual )
{
	...
	if( OH_BitTest( instData->getStyle(), GWS_PUSH_BUTTON ) == FALSE )
	{
	...
	//
	if( OH_BitTest( button->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OH_BitTest( instData->getStyle(), GWS_CHECK_BOX ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OH_BitTest( checkbox->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OH_BitTest( instData->getStyle(), GWS_RADIO_BUTTON ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OH_BitTest( radioButton->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	if( OH_BitTest( instData->getStyle(), GWS_TAB_CONTROL ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OH_BitTest( tabControl->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  ListboxData *listboxData;
	...
	// we MUST have a push button style window to do this
	if( OH_BitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) == FALSE )
	{
  ...
  listbox->winSetUserData( listboxData );
	...
	if( OH_BitTest( listbox->winGetStatus(), WIN_STATUS_IMAGE ))
...

{
	...
	if( OH_BitTest( instData->getStyle(), GWS_HORZ_SLIDER ) ) 
	{
	...
	else if ( OH_BitTest( instData->getStyle(), GWS_VERT_SLIDER ) ) 
	{
		...
		if( OH_BitTest( slider->winGetStatus(), WIN_STATUS_IMAGE ) && !(parent && OH_BitTest(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
																								  ...
																								  Bool defaultVisual )
{
  ...
  ComboBoxData *comboBoxData;
	...
	// we MUST have a push button style window to do this
	if( OH_BitTest( instData->getStyle(), GWS_COMBO_BOX) == FALSE )
	{
...
// begin here
  ...
  comboBox->winSetUserData( comboBoxData );
	...
	// Set up list box redraw callbacks
	if( OH_BitTest( comboBox->winGetStatus(), WIN_STATUS_IMAGE ))
	...
	if( OH_BitTest( comboBox->winGetStyle(), GWS_MOUSE_TRACK ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OH_BitTest( instData->getStyle(), GWS_PROGRESS_BAR ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OH_BitTest( progressBar->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  if( OH_BitTest( instData->getStyle(), GWS_STATIC_TEXT ) ) 
	{
  ...
  if( textWin != NULL )
	{
		...
		if( OH_BitTest( textWin->winGetStatus(), WIN_STATUS_IMAGE ) )
    ...
    memcpy( data, textData, sizeof(TextData) );
		...
		data->text->setWordWrapCentered( OH_BitTest( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
...

{
	...
	if( OH_BitTest( instData->getStyle(), GWS_ENTRY_FIELD ) == FALSE )
	{
	...
	if( OH_BitTest( entry->winGetStatus(), WIN_STATUS_IMAGE ) )
																								 ...
																								 Bool assignVisual )
{
	...
	if( OH_BitTest( instData->getStyle(), GWS_PUSH_BUTTON ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_CHECK_BOX ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_RADIO_BUTTON ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_HORZ_SLIDER ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_COMBO_BOX ) )
	{
//		ComboBoxData *comboBoxData = (ComboBoxData *)gadget->winGetUserData();
	...
	else if( OH_BitTest( instData->getStyle(), GWS_PROGRESS_BAR ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_STATIC_TEXT ) )
	{
	...
	else if( OH_BitTest( instData->getStyle(), GWS_ENTRY_FIELD ) )
	{
...
GameWindow *GameWindowManager::getWindowUnderCursor( Int x, Int y, Bool ignoreEnabled )
{
	...
	else
	{
		...
		for( window = m_windowList; window; window = window->m_next )
		{
			...
			{
				if( OH_BitTest( window->m_status, WIN_STATUS_ENABLED ) || ignoreEnabled )
				{
					// determine which child window the mouse is in
	...
	if( window )
	{
		...
		else if( ignoreEnabled && !( OH_BitTest( window->m_status, WIN_STATUS_ENABLED ) ))
		{
			window = NULL;
		}
	}
