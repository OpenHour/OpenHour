...
void Keyboard::createStreamMessages( void )
{
	...
	while( key->key != KEY_NONE )
	{
		...
		if( OH_BitTest( key->state, KEY_STATE_DOWN ) )
		{
		...
		else if( OH_BitTest( key->state, KEY_STATE_UP ) )
		{
...
void Keyboard::updateKeys( void )
{
	...
	while( m_keys[ index ].key != KEY_NONE )
	{
		...
		{
			if( OH_BitTest( m_keyStatus[ KEY_LALT ].state, KEY_STATE_DOWN ) ||
					OH_BitTest( m_keyStatus[ KEY_RALT ].state, KEY_STATE_DOWN ) )
			{
...
//		return FALSE;
	...
	for( key = 0; key < 256; key++ )
	{
		...
		if( OH_BitTest( m_keyStatus[ key ].state, KEY_STATE_DOWN ) )
		{
