...
GameMessage::Type CommandTranslator::issueSpecialPowerCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos, Object* ignoreSelObj )
{
	...
	if( OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
	...
	else if( OH_BitTest( command->getOptions(), NEED_TARGET_POS ) )
	{
...
GameMessage::Type CommandTranslator::issueCombatDropCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos )
{
	...
	if( target != NULL && OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
...
GameMessage::Type CommandTranslator::issueFireWeaponCommand( const CommandButton *command, CommandEvaluateType commandType, Drawable *target, const Coord3D *pos )
{
	...
	if( OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
	{
		...
		if( OH_BitTest( command->getOptions(), ATTACK_OBJECTS_POSITION ) )
		{
																														 ...
																														 CommandEvaluateType type )
{
	...
	if (obj == NULL || 
		(OH_BitTest(obj->getStatusBits(), OBJECT_STATUS_MASKED) && 
			...
			|| (command && command->getCommandType() == GUI_COMMAND_SPECIAL_POWER_FROM_COMMAND_CENTER))
	{
		...
		{
			if( obj && obj->isKindOf( KINDOF_SHRUBBERY ) && !OH_BitTest( command->getOptions(), ALLOW_SHRUBBERY_TARGET ) )
			{
			...
			if( obj && obj->isKindOf( KINDOF_MINE ) && !OH_BitTest( command->getOptions(), ALLOW_MINE_TARGET ) )
			{
			...
			if( obj && OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
			{
				Relationship relationship = ThePlayerList->getLocalPlayer()->getRelationship( obj->getTeam() );
				switch( relationship )
				{
					case ALLIES:
						if( !OH_BitTest( command->getOptions(), NEED_TARGET_ALLY_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
						break;
					case ENEMIES:
						if( !OH_BitTest( command->getOptions(), NEED_TARGET_ENEMY_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
						break;
					case NEUTRAL:
						if( !OH_BitTest( command->getOptions(), NEED_TARGET_NEUTRAL_OBJECT ) )
						{
							draw = NULL;
							obj = NULL;
						}	
