...
void InGameUI::objectChangedTeam(const Object *obj, Int oldPlayerIndex, Int newPlayerIndex)
{
	...
	if (obj && oldPlayerIndex >= 0 && newPlayerIndex >= 0)
	{
		...
		for (BehaviorModule** m = obj->getBehaviorModules(); *m; ++m)
		{
			...
			if (!found)
			{
				if( TheGameLogic->getFrame() == 0 && OH_BitTest( obj->getStatusBits(), OBJECT_STATUS_UNDER_CONSTRUCTION ) == FALSE &&
		...
		case MOUSEMODE_GUI_COMMAND:
			{
				...
				if( m_pendingGUICommand )
				{
					...
					else if( OH_BitTest( m_pendingGUICommand->getOptions(), COMMAND_OPTION_NEED_TARGET ) )
					{
...
void InGameUI::setGUICommand( const CommandButton *command )
{
	...
	if( command )
	{
		...
		if( OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_TARGET ) == FALSE )
		{
	...
	// set the mouse cursor for commands that need a targeting or to normal with no command
	if( command && OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_TARGET ) && !command->isContextCommand() )
	{
...
Bool InGameUI::canSelectedObjectsDoSpecialPower( const CommandButton *command, const Object *objectToInteractWith, const Coord3D *position, SelectionRules rule, UnsignedInt commandOptions, Object* ignoreSelObj ) const
{
	...
	//3) NEED_TARGET_POS
	Bool doAtPosition = OH_BitTest( command->getOptions(), NEED_TARGET_POS );
	Bool doAtObject = OH_BitTest( command->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET );
			 ...
			 it != m_worldAnimationList.end(); /*empty*/ )
	{
		...
		if( TheGameLogic->isGamePaused() == FALSE )
		{
			...
			if( TheGameLogic->getFrame() >= wad->m_expireFrame ||
					(OH_BitTest( wad->m_options, WORLD_ANIM_PLAY_ONCE_AND_DESTROY ) &&
					 OH_BitTest( wad->m_anim->getStatus(), ANIM_2D_STATUS_COMPLETE )) )
			{
		...
		if( OH_BitTest( wad->m_options, WORLD_ANIM_FADE_ON_EXPIRE ) )
		{
