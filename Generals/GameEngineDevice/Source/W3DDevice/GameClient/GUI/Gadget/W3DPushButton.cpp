																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( OH_BitTest( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !OH_BitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			{
				if( !OH_BitTest( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !OH_BitTest( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(OH_BitTest(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
				if (OH_BitTest( instData->getState(), WIN_STATE_HILITED ))
				{
					if( OH_BitTest( instData->getState(), WIN_STATE_SELECTED ) )
					{
  			...
  			else if( OH_BitTest( instData->getState(), WIN_STATE_SELECTED ) )
 				{
