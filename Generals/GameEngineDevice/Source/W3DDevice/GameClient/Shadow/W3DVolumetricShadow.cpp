	...
	for( i = 0; i < numPolys; i++ )
	{
		...
		for( j = 0; j < MAX_POLYGON_NEIGHBORS; j++ )
		{
			...
			if( polyNeighbor->neighbor[ j ].neighborIndex != NO_NEIGHBOR )
			{
				...
				//
				if( OH_BitTest( otherNeighbor->status, POLY_PROCESSED ) )
					continue;  // for j
			...
			//
			if( OH_BitTest( polyNeighbor->status, POLY_VISIBLE ) )
			{
				...
				else if( OH_BitTest( otherNeighbor->status, POLY_VISIBLE ) == FALSE )
				{
					...
					addSilhouetteEdge(meshIndex, polyNeighbor, otherNeighbor );
							 ...
							 OH_BitTest( otherNeighbor->status, POLY_VISIBLE ) )
			{
