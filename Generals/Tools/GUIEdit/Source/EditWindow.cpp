														 ...
														 WPARAM wParam, LPARAM lParam )
{
	Int x = LOWORD( lParam );
	Int y = HIWORD( lParam );
	Bool controlDown = OH_BitTest( GetKeyState( VK_CONTROL ), 0x1000 );
	ICoord2D mouse;
...
void EditWindow::drawSeeThruOutlines( GameWindow *windowList, Color c )
{
	...
	if( OH_BitTest( windowList->winGetStatus(), WIN_STATUS_SEE_THRU ) )
	{
		ICoord2D pos;
		ICoord2D size;
...
void EditWindow::drawHiddenOutlines( GameWindow *windowList, Color c )
{
	...
	while( parent )
	{
		...
		if( OH_BitTest( parent->winGetStatus(), WIN_STATUS_HIDDEN ) )
			hidden = TRUE;
		parent = parent->winGetParent();

	}  // end while
	if( OH_BitTest( windowList->winGetStatus(), WIN_STATUS_HIDDEN ) )
		hidden = TRUE;
	if( hidden )
	{
		ICoord2D pos;
														...
														Color color )
{
	...
	if( OH_BitTest( image->getStatus(), IMAGE_STATUS_ROTATED_90_CLOCKWISE ) )
	{
