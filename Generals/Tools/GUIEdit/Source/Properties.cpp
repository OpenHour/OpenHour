...
void InitPropertiesDialog( GameWindow *window, Int x, Int y )
{
	...
	else if( OH_BitTest( window->winGetStyle(), GWS_TAB_CONTROL ) )
	...
	else if( OH_BitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
		dialog = InitListboxPropertiesDialog( window );
	else if( OH_BitTest( window->winGetStyle(), GWS_PROGRESS_BAR ) )
	...
	else if( OH_BitTest( window->winGetStyle(), GWS_ALL_SLIDER ) )
		dialog = InitSliderPropertiesDialog( window );
	else if( OH_BitTest( window->winGetStyle(), GWS_COMBO_BOX ) )
...
void LoadStateCombo( UnsignedInt style, HWND comboBox )
{
	...
	for( entry = imageAndColorTable; entry->stateName; entry++ )
	{
		...
		if( OH_BitTest( entry->windowType, style ) )
		{
...
void CommonDialogInitialize( GameWindow *window, HWND dialog )
{
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) )
		CheckDlgButton( dialog, CHECK_ENABLED, BST_CHECKED );
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_DRAGABLE ) )
		CheckDlgButton( dialog, CHECK_DRAGABLE, BST_CHECKED );
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_HIDDEN ) )
		CheckDlgButton( dialog, CHECK_HIDDEN, BST_CHECKED );
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_NO_INPUT ) )
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_BORDER ) )
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_SEE_THRU ) )
	...
	if( OH_BitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
...
Bool SaveCommonDialogProperties( HWND dialog, GameWindow *window )
{
	...
	if( TheEditor->windowIsGadget( window ) )
		adjustGadgetDrawMethods( OH_BitTest( window->winGetStatus(), WIN_STATUS_IMAGE ),
														 window );
