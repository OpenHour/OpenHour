...
static GameWindow *pointInChild( Int x, Int y , GameWindow *win)
{
	...
	for( child = win->winGetChild(); child; child = child->winGetNext() ) 
	{
		...
		if( x >= origin.x && x <= origin.x + tempX &&
				...
				OH_BitTest( child->winGetStatus(), WIN_STATUS_HIDDEN ) == FALSE)
...
static GameWindow *pointInAnyChild( Int x, Int y, Bool ignoreHidden, GameWindow *win )
{
	...
	for( child = win->winGetChild(); child; child = child->winGetNext() ) 
	{
				...
				y >= origin.y && y <= origin.y + tempY )
		{
			...
			if( !(ignoreHidden == TRUE &&	OH_BitTest( child->winGetStatus(), WIN_STATUS_HIDDEN )) )
...
GameWindow *GUIEdit::getWindowAtPos( Int x, Int y )
{
	...
	if( pick )
	{
		...
		if( parent )
		{
			...
			if( OH_BitTest( parent->winGetStyle(), GWS_VERT_SLIDER |
																					GWS_HORZ_SLIDER |
																					GWS_SCROLL_LISTBOX |
																					GWS_COMBO_BOX |
																					GWS_TAB_CONTROL
				...
				)
			{
				...
				//
				if( OH_BitTest( parent->winGetStyle(), GWS_HORZ_SLIDER | 
																						GWS_VERT_SLIDER ) )
				{
					GameWindow *grandParent = parent->winGetParent();

					if( grandParent && OH_BitTest( grandParent->winGetStyle(),
																			GWS_SCROLL_LISTBOX ) )
						pick = grandParent;
				...
				if(OH_BitTest(pick->winGetStyle(), GWS_SCROLL_LISTBOX))
				{
					GameWindow *grandParent = pick->winGetParent();
					if( grandParent && OH_BitTest( grandParent->winGetStyle(), GWS_COMBO_BOX))
						pick = grandParent;
				}					
			}  // end if
...
Bool GUIEdit::windowIsGadget( GameWindow *window )
{
	...
	return OH_BitTest( window->winGetStyle(), GWS_GADGET_WINDOW );
