...
static Bool saveType( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	if( OH_BitTest( window->winGetStyle(), GWS_PUSH_BUTTON ) )
		...
		type = "TABCONTROL";
	else if( OH_BitTest( window->winGetStyle(), GWS_TAB_PANE ) )
		type = "TABPANE";
	else if( OH_BitTest( window->winGetStyle(), GWS_CHECK_BOX ) )
		type = "CHECKBOX";
	else if( OH_BitTest( window->winGetStyle(), GWS_VERT_SLIDER ) )
		type = "VERTSLIDER";
	else if( OH_BitTest( window->winGetStyle(), GWS_HORZ_SLIDER ) )
		type = "HORZSLIDER";
...
static Bool saveStatus( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	while( WindowStatusNames[ i ] )
	{
		...
		bit = 1 << i;
		if( OH_BitTest( window->winGetStatus(), bit ) )
		{
...
static Bool saveStyle( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	while( WindowStyleNames[ i ] )
	{
		...
		bit = 1 << i;
		if( OH_BitTest( window->winGetStyle(), bit ) )
		{
...
//	WinInstanceData *instData = window->winGetInstanceData();
  ...
  saveTooltipDelay( window, fp, dataIndent );
	...
	//
	if( (TheEditor->windowIsGadget( window ) == FALSE)  ||  OH_BitTest(window->winGetStyle(), GWS_TAB_CONTROL) )
	{
...
void GUIEdit::updateRadioScreenIdentifiers( GameWindow *window, Int screenID )
{
	...
	if( OH_BitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
	{
		RadioButtonData *radioData = (RadioButtonData *)window->winGetUserData();
