...
Bool AudioManager::shouldPlayLocally(const AudioEventRTS *audioEvent)
{
	...
	if (!OHBitTest(ei->m_type, (ST_PLAYER | ST_ALLIES | ST_ENEMIES | ST_EVERYONE))) {
		DEBUG_CRASH(("No player restrictions specified for '%s'. Using Everyone.\n", ei->m_audioName.str()));
		return TRUE;
	}

	if (OHBitTest(ei->m_type, ST_EVERYONE)) {
		return TRUE;
	}

	Player *owningPlayer = ThePlayerList->getNthPlayer(audioEvent->getPlayerIndex());

	if (OHBitTest(ei->m_type, ST_PLAYER) && OHBitTest(ei->m_type, ST_UI) && owningPlayer == NULL) {
	...
	if (OHBitTest(ei->m_type, ST_PLAYER))  {
		return owningPlayer == localPlayer;
	}

	if (OHBitTest(ei->m_type, ST_ALLIES)) { 
	...
	if (OHBitTest(ei->m_type, ST_ENEMIES)) {
