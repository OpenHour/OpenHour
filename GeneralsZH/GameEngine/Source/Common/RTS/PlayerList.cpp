...
Player *PlayerList::getEachPlayerFromMask( PlayerMaskType& maskToAdjust )
{
	...
	for( i = 0; i < MAX_PLAYER_COUNT; i++ )
	{
		...
		player = getNthPlayer( i );
		if ( player && OHBitTest(player->getPlayerMask(), maskToAdjust ))
		{
...
PlayerMaskType PlayerList::getPlayersWithRelationship( Int srcPlayerIndex, UnsignedInt allowedRelationships )
{
	...
	if (OHBitTest(allowedRelationships, ALLOW_SAME_PLAYER))
	...
	for ( Int i = 0; i < getPlayerCount(); ++i )
	{
		...
		{
			case ENEMIES:
				if (OHBitTest(allowedRelationships, ALLOW_ENEMIES))
					BitSet(retVal, player->getPlayerMask());
				break;
			case ALLIES:
				if (OHBitTest(allowedRelationships, ALLOW_ALLIES))
					BitSet(retVal, player->getPlayerMask());
				break;
			case NEUTRAL:
				if (OHBitTest(allowedRelationships, ALLOW_NEUTRAL))
					BitSet(retVal, player->getPlayerMask());
				break;
		}
	}
