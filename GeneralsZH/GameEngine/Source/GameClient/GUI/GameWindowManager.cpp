...
Bool GameWindowManager::isEnabled( GameWindow *win )
{
	...
	if( OHBitTest( win->m_status, WIN_STATUS_ENABLED ) == FALSE )
	{
		return FALSE;
	}
...
Bool GameWindowManager::isHidden( GameWindow *win )
{
	...
	if( OHBitTest( win->m_status, WIN_STATUS_HIDDEN ))
	{
		return TRUE;
	}
																					...
																					WindowMsgData mData2 )
{
	...
	if( msg != GWM_DESTROY && OHBitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return MSG_IGNORED;
...
Int GameWindowManager::winSetFocus( GameWindow *window )
{
	...
	if( window )
		if( OHBitTest( window->winGetStatus(), WIN_STATUS_NO_FOCUS) )
			return 0;
		...
		if( m_grabWindow )
		{
			...
			switch( msg )
			{
				...
				case GWM_LEFT_UP:
				{
					...
					else if( OHBitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ))
					{
						winSendInputMsg( m_grabWindow, GWM_LEFT_UP, packedMouseCoords, 0 );
					}
				...
				case GWM_LEFT_DRAG:
				{
					...
					if( OHBitTest( m_grabWindow->m_status, WIN_STATUS_DRAGABLE ) )
					{
		...
		else
		{
			...
			else
			{
				...
				for( window = m_windowList; window; window = window->m_next )
				{
					...
					if( OHBitTest( window->m_status, WIN_STATUS_ABOVE ) &&
							!OHBitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
							...
							mousePos->y <= window->m_region.hi.y)
					{
						...
						if( OHBitTest( window->m_status, WIN_STATUS_ENABLED ) )
						{
				...
				if( window == NULL )
				{
					...
					for( window = m_windowList; window; window = window->m_next )
					{
						...
						if( OHBitTest( window->m_status, WIN_STATUS_BELOW ) &&
								!OHBitTest( window->m_status, WIN_STATUS_HIDDEN ) &&
								mousePos->x >= window->m_region.lo.x &&
								mousePos->x <= window->m_region.hi.x &&
								mousePos->y >= window->m_region.lo.y &&
			...
			if( window )
				if( OHBitTest( window->m_status, WIN_STATUS_NO_INPUT ) )
				{
					if(window->winGetParent() && OHBitTest( window->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
						window = window->winGetParent();
					else
						window = NULL;
				}
...
//							WinActivate( tempWin );
			...
			if( tooltipsOn )
			{
//				if( toolTipWindow && toolTipWindow->winGetParent() && OHBitTest( toolTipWindow->winGetParent()->winGetInstanceData()->getStyle(), GWS_COMBO_BOX ))
...
Int GameWindowManager::drawWindow( GameWindow *window )
{
	...
	if( OHBitTest( window->m_status, WIN_STATUS_HIDDEN ) == FALSE )
	{

		if( !OHBitTest( window->m_status, WIN_STATUS_SEE_THRU ) && window->m_draw )
		...
		if( OHBitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
			if( OHBitTest( window->m_status, WIN_STATUS_BORDER ) == TRUE &&
					!OHBitTest( window->m_status, WIN_STATUS_SEE_THRU ) )
...
Int GameWindowManager::winDestroy( GameWindow *window )
{
	...
	if( OHBitTest( window->m_status, WIN_STATUS_DESTROYED ) )
		return WIN_ERR_OK;
																										 ...
																										 Bool defaultVisual )
{
	...
	if( OHBitTest( instData->getStyle(), GWS_PUSH_BUTTON ) == FALSE )
	{
	...
	//
	if( OHBitTest( button->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OHBitTest( instData->getStyle(), GWS_CHECK_BOX ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OHBitTest( checkbox->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OHBitTest( instData->getStyle(), GWS_RADIO_BUTTON ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OHBitTest( radioButton->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
	...
	if( OHBitTest( instData->getStyle(), GWS_TAB_CONTROL ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OHBitTest( tabControl->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  ListboxData *listboxData;
	...
	// we MUST have a push button style window to do this
	if( OHBitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) == FALSE )
	{
  ...
  listbox->winSetUserData( listboxData );
	...
	if( OHBitTest( listbox->winGetStatus(), WIN_STATUS_IMAGE ))
...

{
	...
	if( OHBitTest( instData->getStyle(), GWS_HORZ_SLIDER ) ) 
	{
	...
	else if ( OHBitTest( instData->getStyle(), GWS_VERT_SLIDER ) ) 
	{
		...
		if( OHBitTest( slider->winGetStatus(), WIN_STATUS_IMAGE ) && !(parent && OHBitTest(parent->winGetStyle(), GWS_SCROLL_LISTBOX)))
																								  ...
																								  Bool defaultVisual )
{
  ...
  ComboBoxData *comboBoxData;
	...
	// we MUST have a push button style window to do this
	if( OHBitTest( instData->getStyle(), GWS_COMBO_BOX) == FALSE )
	{
...
// begin here
  ...
  comboBox->winSetUserData( comboBoxData );
	...
	// Set up list box redraw callbacks
	if( OHBitTest( comboBox->winGetStatus(), WIN_STATUS_IMAGE ))
	...
	if( OHBitTest( comboBox->winGetStyle(), GWS_MOUSE_TRACK ) )
...

{
	...
	// we MUST have a push button style window to do this
	if( OHBitTest( instData->getStyle(), GWS_PROGRESS_BAR ) == FALSE )
	{
	...
	// on the device level of the engine
	//
	if( OHBitTest( progressBar->winGetStatus(), WIN_STATUS_IMAGE ) )
...

{
  ...
  if( OHBitTest( instData->getStyle(), GWS_STATIC_TEXT ) ) 
	{
  ...
  if( textWin != NULL )
	{
		...
		if( OHBitTest( textWin->winGetStatus(), WIN_STATUS_IMAGE ) )
    ...
    memcpy( data, textData, sizeof(TextData) );
		...
		data->text->setWordWrapCentered( OHBitTest( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
...

{
	...
	if( OHBitTest( instData->getStyle(), GWS_ENTRY_FIELD ) == FALSE )
	{
	...
	if( OHBitTest( entry->winGetStatus(), WIN_STATUS_IMAGE ) )
																								 ...
																								 Bool assignVisual )
{
	...
	if( OHBitTest( instData->getStyle(), GWS_PUSH_BUTTON ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_CHECK_BOX ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_RADIO_BUTTON ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_HORZ_SLIDER ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_SCROLL_LISTBOX ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_COMBO_BOX ) )
	{
//		ComboBoxData *comboBoxData = (ComboBoxData *)gadget->winGetUserData();
	...
	else if( OHBitTest( instData->getStyle(), GWS_PROGRESS_BAR ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_STATIC_TEXT ) )
	{
	...
	else if( OHBitTest( instData->getStyle(), GWS_ENTRY_FIELD ) )
	{
...
GameWindow *GameWindowManager::getWindowUnderCursor( Int x, Int y, Bool ignoreEnabled )
{
	...
	else
	{
		...
		for( window = m_windowList; window; window = window->m_next )
		{
			...
			{
				if( OHBitTest( window->m_status, WIN_STATUS_ENABLED ) || ignoreEnabled )
				{
					// determine which child window the mouse is in
	...
	if( window )
	{
		...
		else if( ignoreEnabled && !( OHBitTest( window->m_status, WIN_STATUS_ENABLED ) ))
		{
			window = NULL;
		}
	}
