												...
												GameWindow *except )
{
	...
	//
	if( window != except && OHBitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
	{
														 ...
														 WindowMsgData mData1, WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_LEFT_UP:
		{
			...
			if( OHBitTest( instData->getState(), WIN_STATE_SELECTED ) == FALSE )
			{
				RadioButtonData *radioData = (RadioButtonData *)window->winGetUserData();
			...
			else if( OHBitTest( instData->getState(), WIN_STATE_HILITED ) == FALSE )
			{
