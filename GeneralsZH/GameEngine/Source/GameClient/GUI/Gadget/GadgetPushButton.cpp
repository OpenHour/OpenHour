...
static Bool buttonTriggersOnMouseDown(GameWindow *window)
{
	...
	Bool onDown = OHBitTest( window->winGetStatus(), WIN_STATUS_ON_MOUSE_DOWN);
																						...
																						WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_MOUSE_ENTERING:
		{
			...
			if(window->winGetParent() && OHBitTest(window->winGetParent()->winGetStyle(),GWS_HORZ_SLIDER) )
			{
		...
		case GWM_MOUSE_LEAVING:
		{
			...
			//
			if( OHBitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
		...
		case GWM_LEFT_DOWN:
		{
			...
			if( OHBitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
			{
				...
				if( OHBitTest( instData->m_state, WIN_STATE_SELECTED ) )
					BitClear( instData->m_state, WIN_STATE_SELECTED );
				else
		...
		case GWM_LEFT_UP:
		{
			...
			//
			if( OHBitTest( instData->getState(), WIN_STATE_SELECTED ) &&
					OHBitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
			{
		...
		case GWM_RIGHT_DOWN:
		{
			...
			if( OHBitTest( instData->getStatus(), WIN_STATUS_RIGHT_CLICK ) )
			{
		...
		case GWM_CHAR:
		{
			switch( mData1 )
			{
				...
				case KEY_SPACE:
				{
					...
					if( OHBitTest( mData2, KEY_STATE_UP ) )
					{
...
void GadgetCheckLikeButtonSetVisualCheck( GameWindow *g, Bool checked )
{
	...
	if( OHBitTest( g->winGetStatus(), WIN_STATUS_CHECK_LIKE ) == FALSE )
	{
...
Bool GadgetCheckLikeButtonIsChecked( GameWindow *g )
{
	...
	return OHBitTest( instData->m_state, WIN_STATE_SELECTED );
