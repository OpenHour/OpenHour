...
void Keyboard::createStreamMessages( void )
{
	...
	while( key->key != KEY_NONE )
	{
		...
		if( OHBitTest( key->state, KEY_STATE_DOWN ) )
		{
		...
		else if( OHBitTest( key->state, KEY_STATE_UP ) )
		{
...
void Keyboard::updateKeys( void )
{
	...
	while( m_keys[ index ].key != KEY_NONE )
	{
		...
		{
			if( OHBitTest( m_keyStatus[ KEY_LALT ].state, KEY_STATE_DOWN ) ||
					OHBitTest( m_keyStatus[ KEY_RALT ].state, KEY_STATE_DOWN ) )
			{
...
//		return FALSE;
	...
	for( key = 0; key < 256; key++ )
	{
		...
		if( OHBitTest( m_keyStatus[ key ].state, KEY_STATE_DOWN ) )
		{
