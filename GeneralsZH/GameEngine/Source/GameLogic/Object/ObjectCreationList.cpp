...
protected:
	...
	) const
	{
		...
		if( OHBitTest( m_disposition, INHERIT_VELOCITY ) && sourceObj )
		{
		...
		if( OHBitTest( m_disposition, LIKE_EXISTING ) )
		{
			if (mtx)
      ...
      if ( obj->isKindOf( KINDOF_STRUCTURE ) )
      {
		...
		if( OHBitTest( m_disposition, ON_GROUND_ALIGNED ) )
		{
			chunkPos.z = 99999.0f;
		...
		if( OHBitTest( m_disposition, SEND_IT_OUT ) )
		{
			obj->setOrientation(GameLogicRandomValueReal(0.0f, 2 * PI));
		...
		if( OHBitTest( m_disposition, SEND_IT_FLYING | SEND_IT_UP | RANDOM_FORCE ) )
		{
			if (mtx)
			{
				DUMPMATRIX3D(mtx);
			...
			if (objUp)
			{
				...
				Coord3D force;
				if( OHBitTest( m_disposition, SEND_IT_FLYING ) )
				{
					Real horizForce = 4.0f * m_dispositionIntensity;		// 2
				...
				else if (OHBitTest(m_disposition, SEND_IT_UP) )
				{
		...
		if( OHBitTest( m_disposition, WHIRLING ) )
		{
			PhysicsBehavior* objUp = obj->getPhysics();
			if (objUp)
			{
		...
		if( OHBitTest( m_disposition, FLOATING ) )
		{
