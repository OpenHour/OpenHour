																		...
																		Coord3D *result )
{
	...
	//
	{
		...
		if( OHBitTest(options->flags, FPF_CLEAR_CELLS_ONLY) && cell->getType() != PathfindCell::CELL_CLEAR )
			return FALSE;
	...
	//
	if( OHBitTest( options->flags, FPF_IGNORE_WATER ) == FALSE )
	{
		...
		//
		if( OHBitTest( options->flags, FPF_WATER_ONLY ) && (isUnderwater == FALSE || layer != LAYER_GROUND) )
			return FALSE;
	...
	// object checks
	if( OHBitTest( options->flags, FPF_IGNORE_ALL_OBJECTS ) == FALSE )
	{
...
//	Bool overlap = FALSE;
		...
		for( Object *them = iter->first(); them; them = iter->next() )
		{
			...
			if( options->relationshipObject )
			{
				...
				if( OHBitTest( options->flags, FPF_IGNORE_ALLY_OR_NEUTRAL_UNITS ) == TRUE &&
				...
				if( OHBitTest( options->flags, FPF_IGNORE_ALLY_OR_NEUTRAL_STRUCTURES ) == TRUE &&
						options->relationshipObject->getRelationship( them ) != ENEMIES &&
						them->isKindOf( KINDOF_STRUCTURE ) )
					continue;

				// if this is an enemy unit and we ignore those, do so
				if( OHBitTest( options->flags, FPF_IGNORE_ENEMY_UNITS ) == TRUE &&
						options->relationshipObject->getRelationship( them ) == ENEMIES &&
						(them->isKindOf( KINDOF_INFANTRY ) || them->isKindOf( KINDOF_VEHICLE )) )
					continue;

				// if this is an enemy structure and we ignore those, do so
				if( OHBitTest( options->flags, FPF_IGNORE_ENEMY_STRUCTURES ) == TRUE &&
						options->relationshipObject->getRelationship( them ) == ENEMIES &&
						them->isKindOf( KINDOF_STRUCTURE ) )
					continue;
																					 ...
																					 Coord3D *result )
{
	...
	DEBUG_ASSERTCRASH( !(OHBitTest( options->flags, FPF_IGNORE_WATER ) == TRUE &&
										   OHBitTest( options->flags, FPF_WATER_ONLY ) == TRUE),
...
void PartitionManager::doShroudReveal(Real centerX, Real centerY, Real radius, PlayerMaskType playerMask) 
{
	...
	for( Int currentIndex = ThePlayerList->getPlayerCount() - 1; currentIndex >=0; currentIndex-- )
	{
		...
		if( OHBitTest( playerMask, currentPlayer->getPlayerMask() ) )
		{
...
void PartitionManager::getMostValuableLocation( Int playerIndex, UnsignedInt whichPlayerTypes, ValueOrThreat valType, Coord3D *outLocation )
{
	...
	for (i = 0; i < cellCount; ++i) {
		...
		for (Int player = 0; player < MAX_PLAYER_COUNT; ++player) {
			if (OHBitTest(allPlayerMasks[player], playerMask)) {
...
static int cellValueProc(PartitionCell* cell, void* userData)
{
	...
	UnsignedInt val = 0;
	for (Int i = 0; i < MAX_PLAYER_COUNT; ++i) {
		if (OHBitTest(parms->allowedPlayersMasks, parms->allPlayersMask[i])) {
