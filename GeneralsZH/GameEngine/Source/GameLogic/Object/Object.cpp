...
//		if (behavior && isAnyKindOf( immuneToShockwaveKindofs ) == FALSE )//NEW RESTRICTIONS ADDED
	...
	if( damageInfo->out.m_actualDamageDealt > 0.0f &&
			...
			!OHBitTest(damageInfo->in.m_sourcePlayerMask, getControllingPlayer()->getPlayerMask()) && 
			m_radarData != NULL &&
    ...
    if ( !( exists && valStr.isEmpty() ) )
    {
      ...
      if ( exists && valBool )
      {
        ...
        if ( audioToModify != NULL )
        {
          ...
          if ( exists && OHBitTest( audioToModify->m_control, AC_LOOP ) )
          {
	...
	if( commandButton )
	{
		switch( commandButton->getCommandType() )
		{
			...
			case GUI_COMMAND_FIRE_WEAPON:
				if( ai )
				{
					if( !OHBitTest( commandButton->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) && !OHBitTest( commandButton->getOptions(), NEED_TARGET_POS ) )
					{
			...
			case GUI_COMMAND_FIRE_WEAPON:
				if( ai )
				{
					if( OHBitTest( commandButton->getOptions(), COMMAND_OPTION_NEED_OBJECT_TARGET ) )
					{
						...
						if( OHBitTest( commandButton->getOptions(), ATTACK_OBJECTS_POSITION ) )
						{
	...
	if( commandButton )
	{
		switch( commandButton->getCommandType() )
		{
			...
			case GUI_COMMAND_FIRE_WEAPON:
				if( ai )
				{
					if( OHBitTest( commandButton->getOptions(), NEED_TARGET_POS ) )
					{
						//LOCATION BASED FIRE WEAPON
						if( !pos )
						{
...
void Object::doCommandButtonUsingWaypoints( const CommandButton *commandButton, const Waypoint *way, CommandSourceType cmdSource )
{
	...
	if( commandButton )
	{
		if( !OHBitTest( commandButton->getOptions(), CAN_USE_WAYPOINTS ) )
		{
