...
public:
	...
	*/
	...
	Bool getIsUndetectedDefector(void) const { return OHBitTest(m_privateStatus, UNDETECTED_DEFECTOR); }
	void friend_setUndetectedDefector(Bool status);

	inline Bool isOffMap() const { return OHBitTest(m_privateStatus, OFF_MAP); }

	inline Bool isCaptured() const { return OHBitTest(m_privateStatus, CAPTURED); }
  ...
  StealthUpdate*          getStealth() const { return m_stealth; }
	...
	inline Bool testScriptStatusBit(ObjectScriptStatusBit b) const { return OHBitTest(m_scriptStatus, b); }
