																		 ...
																		 Int srcX, Int srcY )
{
	...
	if( srcY < imageHeight / 2 &&
			(srcY != 0 || OHBitTest( fitBits,	ImageInfo::FIT_YBORDER_TOP )) )
	{
		...
		row = src + (buffWidth * buffBPP);
					 ...
					 (srcY != imageHeight - 1 || 
						OHBitTest( fitBits, ImageInfo::FIT_YBORDER_BOTTOM )) )
	{
																		...
																		Bool extendAlpha )
{
	...
	if( OHBitTest( image->m_status, ImageInfo::ROTATED90C ) )
	{
	...
	for( y = 0; y < imageHeight; y++ )
	{
		...
		for( x = 0; x < imageWidth; x++ )
		{
			...
			if( currPixel == TRUE && x == imageWidth - 1 &&
					OHBitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_RIGHT ) )
			{
			...
			if( prevPixel == FALSE && currPixel == TRUE )
			{
				...
				//
				if( x != 0 || OHBitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
				{
			...
			if( currPixel == TRUE )
			{
				...
				// top left corner
				if( x == 0 && y == 0 && 
						OHBitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_LEFT ) &&
						OHBitTest( image->m_fitBits, ImageInfo::FIT_YBORDER_TOP ) )
				...
				// bottom right corner
								 ...
								 OHBitTest( image->m_fitBits, ImageInfo::FIT_XBORDER_RIGHT ) &&
								 OHBitTest( image->m_fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					dst = (ptr - (destWidth * destBPP)) + destBPP;
																...
																ImageInfo *image )
{
	...
	if( OHBitTest( image->m_status, ImageInfo::ROTATED90C ) == FALSE )
	{
	...
	//
	if( OHBitTest( TheImagePacker->getGapMethod(), 
...
Bool TexturePage::addImage( ImageInfo *image )
{
	...
	useGutter = OHBitTest( TheImagePacker->getGapMethod(), 
											 ImagePacker::GAP_METHOD_GUTTER );
	useRGBExtend = OHBitTest( TheImagePacker->getGapMethod(),
													ImagePacker::GAP_METHOD_EXTEND_RGB );
	...
	while( triesLeft )
	{
		...
		for( y = 0; y < m_size.y; y++ )
		{
			...
			for( x = 0; x < m_size.x; x++ )
			{
				...
				if( OHBitTest( fitBits, ImageInfo::FIT_XBORDER_LEFT ) )
					image->m_pagePos.lo.x++;
				if( OHBitTest( fitBits, ImageInfo::FIT_YBORDER_TOP ) )
					image->m_pagePos.lo.y++;
				if( OHBitTest( fitBits, ImageInfo::FIT_XBORDER_RIGHT ) )
					image->m_pagePos.hi.x--;
				if( OHBitTest( fitBits, ImageInfo::FIT_YBORDER_BOTTOM ) )
					image->m_pagePos.hi.y--;
				if( OHBitTest( fitBits, ImageInfo::FIT_XGUTTER ) )
					image->m_pagePos.hi.x -= xGutter;
				if( OHBitTest( fitBits, ImageInfo::FIT_YGUTTER ) )	
