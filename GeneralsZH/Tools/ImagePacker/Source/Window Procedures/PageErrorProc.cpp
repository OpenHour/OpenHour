	...
	switch( message )
	{
		...
		case WM_INITDIALOG:
		{
					 ...
					 page = page->m_next )
			{
				...
				if( OHBitTest( page->m_status, TexturePage::PAGE_ERROR ) )
				{
					
					if( OHBitTest( page->m_status, TexturePage::CANT_ALLOCATE_PACKED_IMAGE ) )
						sprintf( reason, "Can't allocate image memory" );
					else if( OHBitTest( page->m_status, TexturePage::CANT_ADD_IMAGE_DATA ) )
						sprintf( reason, "Can't add image(s) data" );
					else if( OHBitTest( page->m_status, TexturePage::NO_TEXTURE_DATA ) )
						sprintf( reason, "No texture data to write" );
					else if( OHBitTest( page->m_status, TexturePage::ERROR_DURING_SAVE ) )
