...
static Bool saveType( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	if( OHBitTest( window->winGetStyle(), GWS_PUSH_BUTTON ) )
		...
		type = "TABCONTROL";
	else if( OHBitTest( window->winGetStyle(), GWS_TAB_PANE ) )
		type = "TABPANE";
	else if( OHBitTest( window->winGetStyle(), GWS_CHECK_BOX ) )
		type = "CHECKBOX";
	else if( OHBitTest( window->winGetStyle(), GWS_VERT_SLIDER ) )
		type = "VERTSLIDER";
	else if( OHBitTest( window->winGetStyle(), GWS_HORZ_SLIDER ) )
		type = "HORZSLIDER";
...
static Bool saveStatus( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	while( WindowStatusNames[ i ] )
	{
		...
		bit = 1 << i;
		if( OHBitTest( window->winGetStatus(), bit ) )
		{
...
static Bool saveStyle( GameWindow *window, FILE *fp, Int dataIndent )
{
	...
	while( WindowStyleNames[ i ] )
	{
		...
		bit = 1 << i;
		if( OHBitTest( window->winGetStyle(), bit ) )
		{
...
//	WinInstanceData *instData = window->winGetInstanceData();
  ...
  saveTooltipDelay( window, fp, dataIndent );
	...
	if( OHBitTest( window->winGetStyle(), GWS_TAB_CONTROL ) )
	...
	//
	if( (TheEditor->windowIsGadget( window ) == FALSE)  ||  OHBitTest(window->winGetStyle(), GWS_TAB_CONTROL) )
	{
...
void GUIEdit::updateRadioScreenIdentifiers( GameWindow *window, Int screenID )
{
	...
	if( OHBitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
	{
		RadioButtonData *radioData = (RadioButtonData *)window->winGetUserData();
