...
void InitPropertiesDialog( GameWindow *window, Int x, Int y )
{
	...
	if( OHBitTest( window->winGetStyle(), GWS_PUSH_BUTTON ) )
		dialog = InitPushButtonPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_RADIO_BUTTON ) )
		dialog = InitRadioButtonPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_TAB_CONTROL ) )
		dialog = InitTabControlPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_CHECK_BOX ) )
		dialog = InitCheckBoxPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_SCROLL_LISTBOX ) )
		dialog = InitListboxPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_PROGRESS_BAR ) )
		dialog = InitProgressBarPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_STATIC_TEXT ) )
		dialog = InitStaticTextPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_ENTRY_FIELD ) )
		dialog = InitTextEntryPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_ALL_SLIDER ) )
		dialog = InitSliderPropertiesDialog( window );
	else if( OHBitTest( window->winGetStyle(), GWS_COMBO_BOX ) )
...
static void saveTextLabel( HWND edit, GameWindow *window )
{
	...
	if( OHBitTest( style, GWS_PUSH_BUTTON ) )
		GadgetButtonSetText( window, text );
	else if( OHBitTest( style, GWS_CHECK_BOX ) )
		GadgetCheckBoxSetText( window, text );
	else if( OHBitTest( style, GWS_RADIO_BUTTON ) )
		GadgetRadioSetText( window, text );
	else if( OHBitTest( style, GWS_STATIC_TEXT ) )
		GadgetStaticTextSetText( window, text );
	else if( OHBitTest( style, GWS_ENTRY_FIELD ) )
...
void LoadStateCombo( UnsignedInt style, HWND comboBox )
{
	...
	for( entry = imageAndColorTable; entry->stateName; entry++ )
	{
		...
		if( OHBitTest( entry->windowType, style ) )
		{
...
void CommonDialogInitialize( GameWindow *window, HWND dialog )
{
	...
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) )
		CheckDlgButton( dialog, CHECK_ENABLED, BST_CHECKED );
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_DRAGABLE ) )
		CheckDlgButton( dialog, CHECK_DRAGABLE, BST_CHECKED );
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_HIDDEN ) )
		CheckDlgButton( dialog, CHECK_HIDDEN, BST_CHECKED );
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_NO_INPUT ) )
	...
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_BORDER ) )
		CheckDlgButton( dialog, CHECK_BORDER, BST_CHECKED );
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_IMAGE ) )
	...
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_CHECK_LIKE ) )
...
static void adjustGadgetDrawMethods( Bool useImages, GameWindow *window )
{
	...
	if( TheEditor->windowIsGadget( window ) )//The below only applies to gadgets.
	{
		...
		if( useImages )
		{
			...
			else if( OHBitTest( style, GWS_TAB_CONTROL ) )
...
Bool SaveCommonDialogProperties( HWND dialog, GameWindow *window )
{
	...
	if( TheEditor->windowIsGadget( window ) )
		adjustGadgetDrawMethods( OHBitTest( window->winGetStatus(), WIN_STATUS_IMAGE ),
														 window );
