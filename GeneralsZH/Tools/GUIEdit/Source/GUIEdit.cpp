...
static GameWindow *pointInChild( Int x, Int y , GameWindow *win)
{
	...
	for( child = win->winGetChild(); child; child = child->winGetNext() ) 
	{
		...
		if( x >= origin.x && x <= origin.x + tempX &&
				...
				OHBitTest( child->winGetStatus(), WIN_STATUS_HIDDEN ) == FALSE)
...
static GameWindow *pointInAnyChild( Int x, Int y, Bool ignoreHidden, GameWindow *win )
{
	...
	for( child = win->winGetChild(); child; child = child->winGetNext() ) 
	{
				...
				y >= origin.y && y <= origin.y + tempY )
		{
			...
			if( !(ignoreHidden == TRUE &&	OHBitTest( child->winGetStatus(), WIN_STATUS_HIDDEN )) )
...
GameWindow *GUIEdit::getWindowAtPos( Int x, Int y )
{
	...
	if( pick )
	{
		...
		if( parent )
		{
			...
			if( OHBitTest( parent->winGetStyle(), GWS_VERT_SLIDER |
																					GWS_HORZ_SLIDER |
																					GWS_SCROLL_LISTBOX |
																					GWS_COMBO_BOX |
																					GWS_TAB_CONTROL
				...
				)
			{
				...
				//
				if( OHBitTest( parent->winGetStyle(), GWS_HORZ_SLIDER | 
																						GWS_VERT_SLIDER ) )
				{
					GameWindow *grandParent = parent->winGetParent();

					if( grandParent && OHBitTest( grandParent->winGetStyle(),
																			GWS_SCROLL_LISTBOX ) )
						pick = grandParent;
				...
				if(OHBitTest(pick->winGetStyle(), GWS_SCROLL_LISTBOX))
				{
					GameWindow *grandParent = pick->winGetParent();
					if( grandParent && OHBitTest( grandParent->winGetStyle(), GWS_COMBO_BOX))
						pick = grandParent;
				}					
			}  // end if
...
Bool GUIEdit::windowIsGadget( GameWindow *window )
{
	...
	return OHBitTest( window->winGetStyle(), GWS_GADGET_WINDOW );
