...
static void drawButtonText( GameWindow *window, WinInstanceData *instData )
{
	...
	text->setWordWrapCentered( OHBitTest( instData->getStatus(), WIN_STATUS_WRAP_CENTERED ));
	...
	// where to draw
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_SHORTCUT_BUTTON ) )
	{
																	 ...
																	 WinInstanceData *instData )
{
	...
	{
		if( OHBitTest( instData->getState(), WIN_STATUS_USE_OVERLAY_STATES ) )
		{
			ICoord2D size, start;
			// get window position
																	 ...
																	 WinInstanceData *instData )
{
	...
	if( !OHBitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
	...
	if( image )
	{
		...
		{	
			...
			{
				if( !OHBitTest( window->winGetStatus(), WIN_STATUS_NOT_READY ) )
				{
					...
					if( !OHBitTest( window->winGetStatus(), WIN_STATUS_ALWAYS_COLOR ) )
					{
	...
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_FLASHING ) )
	{
	...
	if( OHBitTest( window->winGetStatus(), WIN_STATUS_USE_OVERLAY_STATES ) )
	{
		...
		if( pushedOverlayIcon && hilitedOverlayIcon )
		{
			if(OHBitTest(window->winGetStatus(), WIN_STATUS_ENABLED))
			{
				if (OHBitTest( instData->getState(), WIN_STATE_HILITED ))
				{
					if( OHBitTest( instData->getState(), WIN_STATE_SELECTED ) )
					{
  			...
  			else if( OHBitTest( instData->getState(), WIN_STATE_SELECTED ) )
 				{
